...
def add_temp():
    ...
    st.session_state["new_unit"] = opts[0] if opts else "unit"
...
def add_edit_callback(mn):
    ...
    newrow = {
        "Ingredient":      sel,
        "Quantity":        base_q2,
        "Cost Per Unit":   cpu2,             # <--- unified key
        "Total Cost":      tot2,
        "Input Unit":      st.session_state[f"new_unit_edit_{mn}"]
    }
    st.session_state[f"edit_{mn}"] = pd.concat(
        [df_edit,pd.DataFrame([newrow])],ignore_index=True
    )
    # bump form key to clear edit fields
...
def render():
    ...
    st.info("Build meals by adding ingredients…")
        ...
        if st.session_state["editing_meal"] != mn:
            ...
            with exp:
                ...
                for idx,r in df_edit.iterrows():
                    ...
                    cols[1].number_input("Qty",value=qty_val,min_value=0.0,step=0.1,key=f"qty_{mn}_{idx}")

                    base_type = (
                        ing_df.loc[ing_df["Ingredient"]==r["Ingredient"],"Unit Type"].iloc[0]
                    )
                    unit_opts = get_display_unit_options(base_type)
                    curr      = r["Input Unit"]
                    idx_opt   = unit_opts.index(curr) if curr in unit_opts else 0
                    us        = cols[2].selectbox("Unit",unit_opts,index=idx_opt,key=f"unit_{mn}_{idx}")
                ...
                with st.form(key=st.session_state[f"edit_form_key_{mn}"]):
                    ...
                    u2    = get_display_unit_options(base2.iloc[0]["Unit Type"]) if not base2.empty else ["unit"]
